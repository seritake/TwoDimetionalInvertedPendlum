...
const static double I_z = 1.0/2.0*0.1*R_w*R_w; //(kgm^{2}) the moment of inertia of the wheel around the driving shaft.
...
const static double a[2] = {-3.0*K_t*K_t/(2*R_w*R_w*M_t*R_a), -3.0*K_t*K_t*L_c*L_c/(R_w*R_w*I_z*R_a)};
...
void voltCalculator(vector<int>& duty_ratio, vector<double>& angle, vector<double>& d_angle, vector<double>& x, vector<double>& v){
	Vector3d volt;
	...
	d_y_x = v[0] + (m*l*l + I_pyy) / (m*l) * (1.0+tan(angle[0]/2.0)*tan(angle[0]/2.0)) / (1.0-tan(angle[0]/2.0)*tan(angle[0]/2.0))  * d_angle[0]; 
	d_x_y = v[1] + (m*l*l + I_pxx) / (m*l) * (1.0+tan(angle[1]/2.0)*tan(angle[1]/2.0)) / (1.0-tan(angle[1]/2.0)*tan(angle[1]/2.0))  * d_angle[1];
